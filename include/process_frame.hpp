#ifndef _PROCESS_FRAME_HPP_
#define _PROCESS_FRAME_HPP_
...
class process_frame : public receive_frame{
    public:   
        void visual_debug();  
        void stereo_total();
        void stereo_matchBM(cv::Mat rectify_left,cv::Mat rectify_right,cv::Rect ROI1,cv::Rect ROI2);
        void stereo_matchSGBM(cv::Mat rectify_left,cv::Mat rectify_right);
        void resolve_coodinates();
        void stereoBinaryBM(cv::Mat rectify_left,cv::Mat rectify_right);                            // SSE2指令优化
        void stereoBinarySGBM(cv::Mat rectify_left,cv::Mat rectify_right);                           // SSE2指令优化
    private:
        /*矫正过程中的局部变量*/
       ...
       /*SSE2 BM & SGBM*/
       cv::Ptr<cv::stereo::StereoBinaryBM> Binarybm;
       cv::Ptr<cv::stereo::StereoBinarySGBM> Binarysbgm;
       
       /*坐标解析局部变量*/
       std::ofstream of;                          // 读写文件
       /*调试及功能块变量*/
       bool saveButton;                             // 储存参数
       bool loadButton;                             // 加载参数
       bool calibButton;                            // 双目标定
